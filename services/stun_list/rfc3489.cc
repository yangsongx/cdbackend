...
#define UNMATCH_TRANSACTIONID  1

/**
 *
 * return 0 means this is a correct STUN package
 */
...
/**
 * Check if the sepcify stun server is active or not
 *
 * @stun_server : the string of stun server(such as 'stun.hello.com')
 *
 * return 0 means @stun_server is OK, otherwise that server is down.
 */
int check_available(const char *stun_server)
{
    int udp_sock;
    int rc;
    ...
    while(1) {
        ...
        else
        {
            ...
            {
                test_result = process_udp_backdata(udp_sock, (struct sockaddr *)&stunserver_addr);
                if(test_result != UNMATCH_TRANSACTIONID)
                    break;
            }
        }
    }
