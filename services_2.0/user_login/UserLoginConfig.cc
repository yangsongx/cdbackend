...
int UserLoginConfig::prepare_job()
{
    int ret = -1;
...
int UserLoginConfig::init(const char *config_file)
{
    ...
    if(doc != NULL)
    {
        ...
        if(ctx != NULL)
        {
            get_node_via_xpath("//service_2/user_login_service/sqlserver/ip",
                    ctx, buffer, sizeof(buffer));
            strncpy(m_strSqlIP, buffer, sizeof(m_strSqlIP));

            get_node_via_xpath("//service_2/user_login_service/sqlserver/user",
                    ctx, buffer, sizeof(buffer));
            strncpy(m_strSqlUserName, buffer, sizeof(m_strSqlUserName));

            get_node_via_xpath("//service_2/user_login_service/sqlserver/password",
                    ctx, buffer, sizeof(buffer));
            ...
            get_node_via_xpath("//service_2/user_login_service/memcache/ip",
                    ctx, buffer, sizeof(buffer));
            strncpy(m_strMemIP, buffer, sizeof(m_strMemIP));

            get_node_via_xpath("//service_2/user_login_service/memcache/port",
                    ctx, buffer, sizeof(buffer));
...
int UserLoginConfig::reconnect_sql()
{
    int ret = -1;
