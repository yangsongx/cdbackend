...
int VerifyCodeConfig::prepare_job()
{
    int ret = -1;
...
int VerifyCodeConfig::init(const char *config_file)
{
    ...
    if(doc != NULL)
    {
        ...
        if(ctx != NULL)
        {
            ...
            get_node_via_xpath("//service_2/verify_code_service/memcache/ip",
                    ctx, buffer, sizeof(buffer));
            strncpy(m_strMemIP, buffer, sizeof(m_strMemIP));

            get_node_via_xpath("//service_2/verify_code_service/memcache/port",
                    ctx, buffer, sizeof(buffer));
...
int VerifyCodeConfig::reconnect_sql()
{
    int ret = -1;
