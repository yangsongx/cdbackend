...
\frametitle{Web接口设计与定义}
  \framesubtitle{取上传凭证}
单独的bucket存放，前端需要得到上传凭证：

\vspace{0.1cm}

\begin{tikzpicture}
\draw [rounded corners, draw=red!20,fill=red!20, dashed] (6.3, 1.3) rectangle (8, -2.1);
\node [mybox, font=\tiny] (ID1) {浏览器};
\draw (6.5,-1.6) rectangle (7.5, 1.0) node [pos=.5, font=\tiny] {服务器};

%interface fields
\draw [->] ($(ID1.east) + (0cm, 0.3cm)$) -- ($(ID1.east) + (6cm, 0.3cm)$) node [above, pos=.5]{\bgroup
\def\arraystretch{1.15} 
\tiny
\begin{tabular}{|l|l|l|}
\hline
...
\draw [->] ($(ID1.east) + (6cm, -0.3cm)$) -- ($(ID1.east) + (0cm, -0.3cm)$) node [below, pos=.5](IDresponse)
{\bgroup
\def\arraystretch{1.15} 
\tiny
\begin{tabular}{|l|l|}
...
\node [right of = ID1, xshift=2.2cm, font=\tiny] () {URL: /v1/maker/uptoken};
...
\begin{tikzpicture}
\draw [rounded corners, draw=red!20,fill=red!20, dashed] (6.8, 1.3) rectangle (11.8, -3.9);
\node [mybox, font=\tiny] (ID1) {浏览器};
...
\begin{tabular}{|l|l|l|l|l|}
\hline
{cid} & {makerID} & {md5} & {suffix} & {type}\\
\hline
{100} & {21} & {MD5sum} & {png} & {boot-x}\\
\hline
\end{tabular}
\egroup
}; 
...
\node [cylinder , below of = IDserver, yshift=-0.3cm, 
                  aspect=0.25,  draw, minimum width=1.5cm,
                 cylinder body fill=blue!30, 
                 cylinder end fill=blue!20, shape border rotate=90,font=\tiny](IDdb){数据库};

%\draw [->] ($(IDdb.west) + (-3.0cm, 0.4cm)$) -- ($(IDdb.west) + (0, 0.4cm)$) 
\node [below of = IDdb,xshift=1.5cm, font=\tiny]{\bgroup
\def\arraystretch{1.15} 
\tiny
\begin{tabular}{|l|l|l|l|}
\hline
...
\draw [->] (IDdb.south) -- ($(IDdb.south) + (0cm, -0.4cm)$);
\end{tikzpicture}
...
\vspace{0.2cm}
\includegraphics[scale=0.095]{db.jpg}
...
\node [rectangle,right of = IDmaintbl, xshift=3.6cm](IDmakertbl) {\bgroup
\def\arraystretch{1.15} 
\tiny
\begin{tabular}{|l|l|l|l|l|l|}
\hline
{id} & {cid} & {model} & {description} & {PIN码} & {created}\\
\hline
{11} & {100} & {M2C} & {爸爸的M2C} & {AB-XX-XX} & {2015-1-1}\\
{12} & {100} & {M3} & {妈妈的M3} & {} & {2012-1-1}\\
{13} & {101} & {M2D} & {my-1} & {BD-XX-XX} & {2014-06-09}\\
\hline
\end{tabular}
\egroup
}; 
\node[rectangle,font=\tiny, above of = IDmakertbl, yshift=-0.0cm] {uc.maker};
...
\begin{tabular}{|l|l|l|l|}
\hline
{makerID} & {md5(key)} & {suffix} & {type} \\
\hline
{11} & {xx-xxxx} & {jpg} & {boot-1} \\
{11} & {aa-xxxx} & {png} & {boot-2} \\
{11} & {yy-yyyy} & {png} & {墙纸} \\
{12} & {zz-zzzz} & {mp3} & {铃声}  \\
{12} & {cc-xxxx} & {jpg} & {boot-1} \\
{12} & {xx-xxxx} & {jpg} & {boot-2} \\
\hline
\end{tabular}
\egroup
}; 
\node[rectangle,font=\scriptsize, above of = IDmakerfile, yshift=0.1cm,font=\tiny] {uc.makerfiles};
